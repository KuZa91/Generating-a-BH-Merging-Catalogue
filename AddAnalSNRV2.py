...
MPc = 3.086e+25 # MPc to m conversion factor
...
if detector == 'aLIGO':
    ...
    def GetFisco(m1,m2):
        M = m1 + m2
        freq = (1./(6.*np.sqrt(6)*2.*np.pi))*((c*1000)**3.)/(G*M*sol_mass) # Taken from eq 4.39 Maggiore
        return freq
...
if detector == 'LISA':
    ...
    BHCat['AnalSNR_LISA'] = np.sqrt(16.*((A*((BHCat.ChM*sol_mass*G)**(5./6.))/(BHCat.Distance*MPc))**2.)*\
        (1./(c*1000)**3.)*(IntFac(BHCat.LISA_fend) - IntFac(BHCat.InitialFrequency)))

if detector == 'aLIGO':

    BHCat['AnalSNR_aLIGO'] = np.sqrt(16.*(IntFac(BHCat.aLIGO_fend) - IntFac(f_min))*((np.sqrt(5./24.)*((BHCat.ChM*sol_mass*G)**(5./6.)))/(BHCat.Distance*MPc*np.pi**(2./3.)))**2.)
