...
def IMRPhenomD_AnalSNR(Dl, m1, m2):
    ...
    SNR = 4.*simpson((mid_wave**2.)/Sh(mid_frq), mid_frq)
    return np.sqrt(SNR)
...
if __name__ == '__main__':                                    
    ...
    pool = Pool(n_jobs)
    ris = pool.map(AnalSNR_ToPar(i), range(len(BHCat['Mass1'])))
    pool.close()
    pool.join()
...
if detector == 'LISA':
    ...
    CUTBHCat.to_hdf('LISAAnSNRIMRPDB'+SNR_cut+Inc_mode+df_nm, df_key, mode='w')

if detector == 'aLIGO':
    CUTBHCat = BHCat[BHCat['AnalSNRIMRPD_aLIGO'] >= float(SNR_cut)]
    print('The number of sources with SNR bigger than '+str(SNR_cut)+' is '+str(len(CUTBHCat.Mass1)))
    CUTBHCat.to_hdf('aLIGOAnSNRIMRPDB'+SNR_cut+Inc_mode+df_nm, df_key, mode='w')

if detector == 'aplusLIGO':
    CUTBHCat = BHCat[BHCat['AnalSNRIMRPD_aplusLIGO'] >= float(SNR_cut)]
    print('The number of sources with SNR bigger than '+str(SNR_cut)+' is '+str(len(CUTBHCat.Mass1)))
    CUTBHCat.to_hdf('aplusLIGOAnSNRIMRPDB'+SNR_cut+Inc_mode+df_nm, df_key, mode='w')
