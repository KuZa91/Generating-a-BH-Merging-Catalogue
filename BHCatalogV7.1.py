...
if Mass_PDF == 'LIGO_Fid':
    ...
    def P2PS(ran_m2, beta_q, m_min, delta_m):
        ...
        for i in range(len(ran_m1) - 1):
            ...
            for j in range(i + 1):
                ...
                q_norm[i] += ((0.5*(ran_m2[j] + ran_m2[j + 1])/(0.5*(ran_m2[i] + ran_m2[i + 1])))**(beta_q))*                (ran_m2[j + 1] - ran_m1[j])*MassSmoothing(0.5*(ran_m2[j] + ran_m2[j + 1]), m_min, delta_m)
    ...
    def MassDistr(m1, m2, m_min, m_max, alpha, beta_q, delta_m, lambda_peak, mu_m, sigma_m, PL_norm, GS_norm, q_norm, Mass_PS):
        ...
        if(m1 > m2):
            return ((1. - lambda_peak)*MassPowLaw(m1, m_min, m_max, alpha, PL_norm) +                     lambda_peak*MassGauss(m1, mu_m, sigma_m, GS_norm))*MassSmoothing(m1, m_min, delta_m)*                    ((m2/m1)**(beta_q))*(1./q_norm)*MassSmoothing(m2, m_min, delta_m)*(1./Mass_PS)
        else:
            return 0.
        ...
        for i in range(len(ran_m1)- 1):
            for j in range(len(ran_m2)- 1):
                 if(ran_m1[i] >= ran_m2[j]):
                    ...
                    mid_m2 = 0.5*(ran_m2[j + 1] + ran_m2[j])
                    q = mid_m2/mid_m1 
                    ris +=  (ran_m1[i + 1] - ran_m1[i])*(ran_m2[j + 1] - ran_m2[j])*((1. - lambda_peak)                    *MassPowLaw(mid_m1, m_min, m_max, alpha, PL_norm) + lambda_peak*MassGauss(mid_m1, mu_m, sigma_m, GS_norm))                    *MassSmoothing(mid_m1, m_min, delta_m)*(q**(beta_q))*(1./q_norm[i])*MassSmoothing(mid_m2, m_min, delta_m)                    *(1./Mass_PS)
...
def SpinOrientDistrib(cos_t1,cos_t2, zeta, sigma_1, sigma_2):
    prob = (1. - zeta)/(4) + ((2.*zeta)/(np.pi))*    (np.exp(-((np.power(1. - cos_t1,2.))/(2.*np.power(sigma_1,2.))))/(sigma_1*sc.erf(np.sqrt(2)/sigma_1)))    *(np.exp(-((np.power(1. - cos_t2,2.))/(2.*np.power(sigma_2,2.))))/(sigma_2*sc.erf(np.sqrt(2)/sigma_2)))
    return prob
...
def NDistrib(z, m1, m2, Delta_z, m21_norm = []):
    if Mass_PDF == 'LIGO_Fid':
        n = R(z)*DeVC(z, Delta_z)*(T_tot /(1. + z))         *MassDistr(m1, m2, m_min, m_max, alpha, beta_q, delta_m, lambda_peak, mu_m, sigma_m, PL_norm, GS_norm, m21_norm , Mass_PS)
    
    if Mass_PDF == 'LogNorm':
        n = R(z)*DeVC(z, Delta_z)*(T_tot /(1. + z))         *MassDistr(m1, PBH_Mc, PBH_sigmamf, LN_norm)*MassDistr(m2, PBH_Mc, PBH_sigmamf, LN_norm)
        
    if Mass_PDF == 'Gaussian':
            n = R(z)*DeVC(z, Delta_z)*(T_tot /(1. + z))             *MassDistr(m1, mu_m, sigma_m, GS_norm)*MassDistr(m2, mu_m, sigma_m, GS_norm)
...
def Gen_Events_Parameters(idx_m1, idx_m2, idx_z) :
        ...
        if(ev_ifrq <= frq_max):
            ...
            BH = pd.DataFrame([[ev_z, ev_m1, ev_m2, ev_ifrq, ev_ttilde],],                            columns = ['Redshift', 'Mass1', 'Mass2', 'InitialFrequency', 'CoalTime'])
            return BH
        else:
    ...
    for im2 in (range(im1 + 1)):
        ...
        for iz in range(len(ran_z) - 1):
            ...
            if Mass_PDF == 'LIGO_Fid':
                ...
                nstep =  NDistrib(st.mean([ran_z[iz],ran_z[iz + 1]]), st.mean([ran_m1[im1],ran_m1[im1 + 1]]),                                  st.mean([ran_m2[im2],ran_m2[im2 + 1]]), (ran_z[iz +1] - ran_z[iz]), m21_norm = q_norm[im1])
            else:
                
                nstep =  2.*NDistrib(st.mean([ran_z[iz],ran_z[iz + 1]]), st.mean([ran_m1[im1],ran_m1[im1 + 1]]),                                  st.mean([ran_m2[im2],ran_m2[im2 + 1]]), (ran_z[iz +1] - ran_z[iz]))
...
if Check_Plot:
    ...
    for i in range(len(ran_m1) -1):
        for j in range(len(ran_m2) - 1):
            ...
            if Mass_PDF == 'LIGO_Fid':
                Z[j][i] = MassDistr(0.5*(X[j][i] + X[j][i + 1]), 0.5*(Y[j][i] + Y[j + 1][i]), m_min, m_max, alpha, beta_q, delta_m, lambda_peak,                                    mu_m, sigma_m, PLPS, GSPS, q_norm[i], Mass_PS)
            ...
            if(i == (len(ran_m1) -2)):
                ...
                if Mass_PDF == 'LIGO_Fid':
                    Z[j][i+1] = MassDistr(0.5*(X[j][i] + X[j][i + 1]), 0.5*(Y[j][i] + Y[j + 1][i]), m_min, m_max, alpha, beta_q, delta_m, lambda_peak,                                          mu_m, sigma_m, PLPS, GSPS, q_norm[i], Mass_PS)
                if Mass_PDF == 'LogNorm':
            ...
            if(j == (len(ran_m2) -2)):
                ...
                if Mass_PDF == 'LIGO_Fid':                                                                                          
                    Z[j+1][i] = MassDistr(0.5*(X[j][i] + X[j][i + 1]), 0.5*(Y[j][i] + Y[j + 1][i]), m_min, m_max, alpha, beta_q, delta_m, lambda_peak,                                      mu_m, sigma_m, PLPS, GSPS, q_norm[i], Mass_PS)
    ...
    if Mass_PDF == 'LIGO_Fid': 
        Z[len(ran_m2) - 1][len(ran_m1) -1] = MassDistr(0.5*(X[len(ran_m2) - 1][len(ran_m1) -2] + X[len(ran_m2) - 1][len(ran_m1) -1]),                                                       0.5*(Y[len(ran_m2) - 2][len(ran_m1) - 1] + Y[len(ran_m2) - 1][len(ran_m1) -1]),                                                       m_min, m_max, alpha, beta_q, delta_m, lambda_peak, mu_m, sigma_m, PLPS, GSPS,                                                       q_norm[len(ran_m1) -1], Mass_PS)
    if Mass_PDF == 'LogNorm':
        Z[len(ran_m2) - 1][len(ran_m1) -1] = MassDistr(0.5*(X[len(ran_m2) - 1][len(ran_m1) -2] + X[len(ran_m2) - 1][len(ran_m1) -1]),PBH_Mc, PBH_sigmamf, Mass_PS)*                                             MassDistr(0.5*(Y[len(ran_m2) - 2][len(ran_m1) - 1] + Y[len(ran_m2) - 1][len(ran_m1) -1]),PBH_Mc, PBH_sigmamf, Mass_PS)
    
    if Mass_PDF == 'Gaussian':
        Z[len(ran_m2) - 1][len(ran_m1) -1] = MassDistr(0.5*(X[len(ran_m2) - 1][len(ran_m1) -2] + X[len(ran_m2) - 1][len(ran_m1) -1]), mu_m, sigma_m, Mass_PS)*                                             MassDistr(0.5*(Y[len(ran_m2) - 2][len(ran_m1) - 1] + Y[len(ran_m2) - 1][len(ran_m1) -1]), mu_m, sigma_m, Mass_PS)                                                                                                     
